-- Atomic visitor registration: quota check + insert in a single transaction.
-- pg_advisory_xact_lock serializes concurrent requests for the same unit,
-- eliminating the race condition between quota reads and writes.
--
...
BEGIN
  ...
  -- Determine month range of the booking (using UTC month boundaries,
  -- consistent with how the TypeScript monthBounds() function works).
  ...
  WHILE v_iter_month <= v_end_month LOOP
    -- Month boundaries as UTC midnight (matches TypeScript monthBounds behavior)
